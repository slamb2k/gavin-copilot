...
export const useChat = () => {
    ...
    const createChat = async () => {
        const chatTitle = `${process.env.REACT_APP_SITE_TITLE} @ ${new Date().toLocaleString()}`;
        try {
            await chatService
...
export function getFriendlyChatName(convo: ChatState): string {
    ...
    const autoGeneratedTitleRegex = new RegExp(
        `/${process.env.REACT_APP_SITE_TITLE} @ [0-9]{1,2}\/[0-9]{1,2}\/[0-9]{1,4}, [0-9]{1,2}:[0-9]{1,2}:[0-9]{1,2} [A,P]M/`
    );
